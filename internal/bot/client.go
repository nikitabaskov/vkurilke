...
func (c *Client) Send(ctx context.Context, user int64, text string, keyboard Keyboard) (int64, error) {
	...
	err := c.Call(ctx, "sendMessage", map[string]any{"chat_id": user, "text": text, "reply_markup": keyboard, "link_preview_options": map[string]bool{"is_disabled": true}}, &message)
	return message.ID, err
}
func (c *Client) Edit(ctx context.Context, user, message int64, text string, keyboard Keyboard) error {
	err := c.Call(ctx, "editMessageText", map[string]any{"chat_id": user, "message_id": message, "text": text, "reply_markup": keyboard, "link_preview_options": map[string]bool{"is_disabled": true}}, nil)
	var apiErr *APIError
